...
if f'json/{filename}' not in gcs.list_object(bucket_name):
    ...
    ls = float(data['Lintang'].replace(' LS', ''))
    bt = float(data['Bujur'].replace(' BT', ''))
    ...
    csv_data = [time, x, y, ls, bt, magnitude, kedalaman, area, location, potensi]

    # append data into datalake/csv/data.csv
    # if folder is empty
    print('uploading csv file ...')
    if f'csv/dataset.csv' not in gcs.list_object(bucket_name=bucket_name, prefix='csv'):
